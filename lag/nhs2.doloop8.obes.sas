...
obes_cav=0; 
pt_obesca=irt8a{i+1}-irt8a{i};
if  obes_ca=1 and irt8a{i}<dt_obesca<=irt8a{i+1} then do;
    obes_cav=1; pt_obesca=dt_obesca-irt8a{i};
end;
if irt8a{i}<=dtdth<irt8a{i+1} then pt_obesca=min(pt_obesca,dtdth-irt8a{i});
...
/***************  Each questionnaire cycle exclusions *****************/
       ...
       %exclude(irt8a{i-1} lt dt_obesca le irt8a{i});
