...
data pre_pm;
   ...
   * add lagged years;
   ...
   /***Must create variable 'lastq' before correcting irtXX.
	If lost to followup , then lastq=last irt.
	if did not lost to follow up, then lastq=.
    */   
    do i=1 to DIM(irt)-1;
...
type2dbv=0; 
pt_diab=irt8a{i+1}-irt8a{i};
if  type2db=1 and irt8a{i}<dt_diab<=irt8a{i+1} then do;
    type2dbv=1; pt_diab=dt_diab-irt8a{i};
end;
if irt8a{i}<=dtdth<irt8a{i+1} then pt_diab=min(pt_diab,dtdth-irt8a{i});
...
/****************  BASELINE EXCLUSIONS ********************************************/
   ...
   %exclude(lastq eq irt8a{i});         /*only returned baseline qq*/  
...
/***************  Each questionnaire cycle exclusions *****************/
       ...
       %exclude(irt8a{i-1} le dt_diab lt irt8a{i});
       %exclude(0 lt lastq le irt8a{i});    	        /*censor lost to follow up*/ 
...
keep 
 ...
 type2db      type2dbv      dt_diab      pt_diab
