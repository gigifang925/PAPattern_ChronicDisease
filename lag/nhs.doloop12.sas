...
chrv=0; 
pt_chr=irt12a{i+1}-irt12a{i};
if  chr=1 and irt12a{i}<dt_chr<=irt12a{i+1} then do;
    chrv=1; pt_chr=dt_chr-irt12a{i};
end;
if irt12a{i}<=dtdth<irt12a{i+1} then pt_chr=min(pt_chr,dtdth-irt12a{i});
...
/***************  Each questionnaire cycle exclusions *************************/
       ...
       %exclude(irt12a{i-1} le dt_chr lt irt12a{i});
