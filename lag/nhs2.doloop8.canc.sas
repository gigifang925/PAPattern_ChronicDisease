...
tot_cancerv=0; 
pt_totcancer=irt8a{i+1}-irt8a{i};
if  tot_cancer=1 and irt8a{i}<dt_totcancer<=irt8a{i+1} then do;
    tot_cancerv=1; pt_chr=dt_totcancer-irt8a{i};
end;
if irt8a{i}<=dtdth<irt8a{i+1} then pt_totcancer=min(pt_totcancer,dtdth-irt8a{i});
...
/***************  Each questionnaire cycle exclusions *****************/
       ...
       %exclude(irt8a{i-1} lt dt_totcancer le irt8a{i});
...
keep 
 ...
 tot_cancer    tot_cancerv    pt_totcancer   dt_totcancer    
