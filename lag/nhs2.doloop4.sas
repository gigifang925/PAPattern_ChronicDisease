...
chrv=0; 
pt_chr=irt4a{i+1}-irt4a{i};
if  chr=1 and irt4a{i}<dt_chr<=irt4a{i+1} then do;
    chrv=1; pt_chr=dt_chr-irt4a{i};
end;
if irt4a{i}<=dtdth<irt4a{i+1} then pt_chr=min(pt_chr,dtdth-irt4a{i});
...
/***************  Each questionnaire cycle exclusions *****************/
       ...
       %exclude(irt4a{i-1} lt dt_chr le irt4a{i});
