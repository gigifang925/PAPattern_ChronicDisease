...
type2dbv=0; 
pt_diab=irt12a{i+1}-irt12a{i};
if  type2db=1 and irt12a{i}<dt_diab<=irt12a{i+1} then do;
    type2dbv=1; pt_diab=dt_diab-irt12a{i};
end;
if irt12a{i}<=dtdth<irt12a{i+1} then pt_diab=min(pt_diab,dtdth-irt12a{i});
...
/************************  BASELINE EXCLUSIONS ********************************/
  if i=1 then do;
       ...
       %exclude(lastq eq irt12a{i});         *only returned baseline qq;  
...
/***************  Each questionnaire cycle exclusions *************************/
       ...
       %exclude(irt12a{i-1} le dt_diab lt irt12a{i});
       %exclude(0 lt lastq le irt12a{i});    	    *censor lost to follow up;  
