...
tot_cancerv=0; 
pt_totcancer=irt4a{i+1}-irt4a{i};
if  tot_cancer=1 and irt4a{i}<dt_totcancer<=irt4a{i+1} then do;
    tot_cancerv=1; pt_chr=dt_totcancer-irt4a{i};
end;
if irt4a{i}<=dtdth<irt4a{i+1} then pt_totcancer=min(pt_totcancer,dtdth-irt4a{i});
...
/***************  Each questionnaire cycle exclusions *****************/
       ...
       %exclude(irt4a{i-1} lt dt_totcancer le irt4a{i});
