...
type2dbv=0; 
pt_diab=irt4a{i+1}-irt4a{i};
if  type2db=1 and irt4a{i}<dt_diab<=irt4a{i+1} then do;
    type2dbv=1; pt_diab=dt_diab-irt4a{i};
end;
if irt4a{i}<=dtdth<irt4a{i+1} then pt_diab=min(pt_diab,dtdth-irt4a{i});
...
/* CHRONIC CONDITIONS */
  ...
  /************************  BASELINE EXCLUSIONS ********************************/
   ...
   if i=1 then do;
       ...
       %exclude(lastq eq irt4a{i});          *only returned baseline qq; 
...
/***************  Each questionnaire cycle exclusions *****************/
       ...
       %exclude(irt4a{i-1} lt dt_diab le irt4a{i});
       %exclude(0 lt lastq le irt4a{i});    	         *censor lost to follow up;  
