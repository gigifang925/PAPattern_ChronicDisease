...
chrv=0; 
pt_chr=irt8a{i+1}-irt8a{i};
if  chr=1 and irt8a{i}<dt_chr<=irt8a{i+1} then do;
    chrv=1; pt_chr=dt_chr-irt8a{i};
end;
if irt8a{i}<=dtdth<irt8a{i+1} then pt_chr=min(pt_chr,dtdth-irt8a{i});
...
/***************  Each questionnaire cycle exclusions *****************/
       ...
       %exclude(irt8a{i-1} lt dt_chr le irt8a{i});
