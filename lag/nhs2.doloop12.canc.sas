...
tot_cancerv=0; 
pt_totcancer=irt12a{i+1}-irt12a{i};
if  tot_cancer=1 and irt12a{i}<dt_totcancer<=irt12a{i+1} then do;
    tot_cancerv=1; pt_chr=dt_totcancer-irt12a{i};
end;
if irt12a{i}<=dtdth<irt12a{i+1} then pt_totcancer=min(pt_totcancer,dtdth-irt12a{i});
...
/***************  Each questionnaire cycle exclusions *****************/
       ...
       %exclude(irt12a{i-1} lt dt_totcancer le irt12a{i});
